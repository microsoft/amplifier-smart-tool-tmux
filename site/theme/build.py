...
def page_url(key, args):
    p = PAGES[key]
    if args.local:
        return f'/{p["repository"]}/{p.get("path", "")}'
    owner = args.family_owner if p['owner'] == 'microsoft' and args.family_owner else p['owner']
    return f'https://{owner}.github.io/{p["repository"]}/{p.get("path", "")}'
...
def tool_page(config, args):
    ...
    hero_actions = (''.join(link(page_url(k, args), PAGES[k]['name'], 'button primary' if i == 0 else 'button') for i, k in enumerate(config['hero_tools'])) if config.get('hero_tools') else link('#get-started', 'Get started', 'button primary') + link(repo, 'View on GitHub', 'button'))
    ...
    return f'''<section class="hero tool-hero"><div class="hero-grid"><div><h1>{esc(config['name'])}</h1><p class="promise">{esc(config['promise'])}</p></div><div><p class="lede">{esc(config['description'])}</p><div class="actions">{hero_actions}</div></div></div></section>
...
def main():
    ...
    parser.add_argument('--config', default='site/site.json')
